...
async function main() {
  ...
  // Diagnostic backend is text-only; code-writing CLI backends stay retired.
  const { createAgentRunner } = await import("./lib/agent-runner.mjs");
  const runner = createAgentRunner("deepseek");
  assert.equal(runner.name, "deepseek");
  assert.throws(() => createAgentRunner("codex"), /Supported: deepseek/);
  assert.throws(() => createAgentRunner("claude"), /Supported: deepseek/);
  ...
  const result = {
    ...
    checks: ["classify-alert", "incident-key", "dispatch-payload-size", "healthcheck-dry-run", "coolify-dry-run", "volc-dry-run", "diagnostic-backend-boundary", "health-poller-interface", "disk-policy", "disk-classify-alert", "disk-safety-guard"],
