...
`options_regen_only` 是玩家继续行动入口的确定性短链路，不是主叙事生成链路。它从最近玩家可见叙事中投影下一步可点击行动，不调用模型、不推进回合、不写世界状态。
...
- P75 ≤ 4s
- P95 ≤ 5s
- 所有选项维护 client deadline：5s
- 服务端 options-only 总预算：5s
- 模型调用次数：0
- repair pass 次数：0
- 本地兜底选项允许次数：0
...
3. 服务端 fast path：`src/lib/turnEngine/playerTurnWorkflow.ts` 的 `clientPurpose === "options_regen_only"` 分支
...
- `repair_pass_used`（只为历史 analytics 兼容保留，不再由当前链路产生）
...
`options_regen_only` 是辅助请求，确定性投影返回 4 条也必须继续检查：
...
实现位置：`src/lib/turnEngine/playerTurnWorkflow.ts` 的 `options_regen_only` fast path。一次确定性投影和语义过滤后仍不足 4 条时，清空可点击选项并提示玩家手动输入；不得再发起模型或 repair pass。
...
2. 保留 `options_regen_only` 确定性 fast path；不得回滚到模型修复链。
