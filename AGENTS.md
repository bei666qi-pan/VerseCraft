...
- **Unit tests hang due to open Redis handles**: `pnpm test:unit` (256 tests) passes completely but the Node process never exits because `worldKnowledgeCache.test.ts` opens a Redis connection that isn't closed. All tests pass (0 failures); use `timeout 300 npx tsx --test "src/**/*.test.ts"` to force exit, or just accept the hang. CI handles this with its own timeout.
