...
- **AI logical roles vs one-api model names**: App code uses roles `main` / `control` / `enhance` / `reasoner`; env vars `AI_MODEL_*` hold the strings sent to the gateway. **Feature code should use `@/lib/ai/logicalTasks` or the same exports from `@/lib/ai/service`** (e.g. `generateMainReply`, `parsePlayerIntent`) rather than `executeChatCompletion` directly. Switch upstream models in one-api or by changing those env vars — not scattered in business logic. If you see "model not exist" from the gateway, fix the mapping in one-api or env.
- **Unit tests hang due to open Redis handles**: `pnpm test:unit` (726+ tests) passes completely but the Node process never exits because `worldKnowledgeCache.test.ts` opens a Redis connection that isn't closed. All tests pass (0 failures); use `timeout 300 npx tsx --test "src/**/*.test.ts"` to force exit, or just accept the hang. CI handles this with its own timeout.
- **PostgreSQL setup in Cloud Agent VMs**: No Docker needed — install `postgresql` via apt, configure the port to match the injected `DATABASE_URL` (local default is typically `5432`), set the postgres password, create the `versecraft` database, then run `pnpm db:push`. Redis is also needed for full test suite: install `redis-server` via apt and start on port 6379.
- **esbuild/sharp/unrs-resolver build scripts**: pnpm install may warn about ignored build scripts for these packages. This is expected with `pnpm.onlyBuiltDependencies` policy; the app works without them.
