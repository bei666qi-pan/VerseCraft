...
- **PostgreSQL setup in Cloud Agent VMs**: No Docker needed — install `postgresql` via apt, configure the port to match the injected `DATABASE_URL` (currently port 5433), set the postgres password, create the `versecraft` database, then run `pnpm db:push`. Redis is also needed for full test suite: install `redis-server` via apt and start on port 6379.
