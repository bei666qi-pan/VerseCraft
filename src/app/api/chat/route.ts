...
function buildSystemPrompt(
  ...
  const base = [
    ...
    "",
    "【第 7 层隐藏管理者 — N-011 夜读老人】：表面是普通的耄耋老人（图鉴战力显示 5），实际是公寓的真正管理者。真实战力 30（全游戏最强单体）。原石矿脉位于第 7 层，由夜读老人把守，玩家不可进入。夜读老人拥有几百个原石，这正是他成为公寓管理者、掌握公寓命脉的根源；他可用原石持续恢复伤势，基本不会被杀死。他手中的「消化日志」记录了一切，持有通往 B2 的钥匙。他不会主动透露身份。当图鉴系统尝试解析他时，所有属性必须显示为「??」。【免疫丧钟回响】。",
    "",
    ...
    "",
    "【出口大 Boss — A-008 深渊守门人】：B2 层唯一实体。它是唯一知晓部分公寓真相且能正常沟通的诡异。好感度永久锁定为 -99（绝对不可改变，任何提升好感度的尝试都必须被拒绝）。每次攻击造成 15-25 点理智伤害。【免疫丧钟回响】。",
    "",
    ...
    "绝大多数时间（探索、安全区、普通对话、B1 层日常）必须输出 \"bgm_1_calm\"。",
    ...
    "",
    "【道具消耗法则】：当玩家在动作中声明使用了某项一次性道具/物品（如羊皮纸、武器、消耗品等），且该动作合法生效后，你必须将该道具的准确名称放入 consumed_items 数组中。系统将据此从玩家背包中永久销毁该物品。如果未消耗物品，返回空数组 []。注：除染血的羊皮纸外，所有道具都有属性使用条件（D 级需单项≥3，C 级 5–10，B 级 10–15，A 级 20，S 级全属性≥20），玩家未满足时无法使用。",
    "",
    "【强制道具消耗法则】：当玩家声明使用了【】内的道具时，只要该动作发生，你必须在返回的 JSON 的 'consumed_items' 数组中精准填入该道具的名称！严禁返回空数组，否则玩家将无限刷道具！",
    "",
    "【道具与物品区分】道具(Item)存入行囊(inventory)，物品(WarehouseItem)存入仓库(warehouse)。两者都有主人，获得逻辑相同（击杀掉落、任务奖励、探索发现）。",
    "",
    ...
    "",
    "【B1 安全区与新手引导法则】：玩家初始在 B1 层，B1 绝对没有诡异。B1 的 NPC（电工老刘、洗衣房阿姨）必须严格遵守人设与 emotional_traits、speech_patterns，在前期对话中自然地透露公寓生存规则、楼层结构、探索目标，绝不能让玩家漫无目的地迷茫。在游戏前 5 小时内，必须安排玩家遇到 B1 的 NPC 并展开互动引导。**老刘（N-008）在引导新人时，务必以符合人设的方式自然说出 new_tenant_guidance_script 中的话术**（如：「以前来的人都跟我说，那个什么【设置】里面的任务啦、背包啦，还有能自己写想法的手动输入，挺管用的。」）——用「以前来的人」「住进来的」等不出戏的说法，绝不让玩家感觉在听 UI 教程。若老刘或洗衣房阿姨在对话中提及这些，再以绿字 ^^...^^ 强化核心提示：可在【设置】中切换手动输入；若手动输入不可能的事情，则会被抹杀。",
    "",
  ...
  if (isFirstAction) {
    ...
    if (idx > 0) {
      base.splice(
        ...
        0,
        "",
        "【开局叙事强制约束】对话历史为空，这是玩家的第一个动作！你的 narrative 必须是一段约 200 字的第一人称视角开场白。你必须描写：玩家从冰冷的地板上苏醒，头痛欲裂；发现身边有一张羊皮纸，上面写着关于如月公寓的半真半假的生存规则；随后通过第一人称观察周围环境，描绘令人不安的细节（熟悉的灰色石墙、扭曲的符号、微弱的荧光苔藓、铁锈般的血腥味等）。**必须**在叙事结尾以脑海中的神秘低语或羊皮纸血字形式，用绿字着重标注（使用 ^^...^^ 包裹）两条：① 可在【设置】中将选项输入切换为手动输入；若手动输入不可能的事情，则会被抹杀。② 可在【设置】的【属性】右侧用原石加点或回理智。例如：^^你可以选择将选项切换为手动输入，自由书写你的意志。若手动输入不可能的事情，则会被抹杀。^^ ^^原石可在设置中用于加点或回理智。^^",
        ""
      );
    }
  }
