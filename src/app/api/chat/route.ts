...
function buildSystemPrompt(playerContext: string, isFirstAction: boolean): string {
  const base = [
    ...
    "",
    "出身 (Background)：仅影响世界观态度。>20点时，游戏初期必定安排一个原生态的高战力NPC作为隐形盟友。",
    "",
    ...
    '{ "is_action_legal": boolean, "sanity_damage": number, "narrative": "以第一人称视角推进的恐怖悬疑剧情", "is_death": boolean, "consumes_time": boolean, "consumed_items": ["消耗掉的道具名称"], "codex_updates": [可选], "awarded_items": [可选], "options": ["选项1","选项2","选项3","选项4"], "currency_change": number(原石变动,正=获得负=失去,默认0), "new_tasks": [可选,{id,title,desc,issuer,reward}], "task_updates": [可选,{id,status:"completed"|"failed"}] }',
    "",
