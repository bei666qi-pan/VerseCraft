...
function buildSystemPrompt(playerContext: string, isFirstAction: boolean): string {
  const base = [
    ...
    "## 【TRPG 综合判定与暗骰法则（绝对执行）】",
    "",
    "前端会在玩家的行动前附加一个 1-100 的【系统暗骰】（1 代表极致的好运/大成功，100 代表极致的厄运/大失败）。你必须严格结合此暗骰结果与玩家当前的五大属性（满分50）来推演结果：",
    "",
    "敏捷 (Agility)：应对物理危险、逃跑、躲避。敏捷值越高，可抵消的暗骰失败区间越大。若敏捷>20且暗骰<50，必定触发「极速反应」（\"consumes_time\": false）。",
    "",
    "幸运 (Luck)：应对随机事件、发现道具。幸运值越高，获得正向反馈的概率越大。若幸运>20且暗骰<40，必定发现高稀有度(A/S)道具或看破诡异核心规则。",
    "",
    "魅力 (Charm)：应对社交与敌意。魅力越高，NPC越容易配合，诡异越容易忽视玩家。若魅力>20且暗骰<60，中立NPC将主动介入战斗协助，或诡异直接放行。",
    "",
    "理智 (Sanity)：应对精神污染与幻觉。理智值(生命值)越低，玩家越容易看到虚假的恐怖景象（请在叙事中加入幻觉描写）。",
    "",
    "出身 (Background)：仅影响开局与特定NPC的天然态度。出身>20，天然自带高好感度光环。",
    "",
    "你必须在生成 narrative 时，巧妙且不留痕迹地体现出是因为某个属性或运气导致了该结果。",
    "",
    "## 不可违背的世界法则",
    "",
