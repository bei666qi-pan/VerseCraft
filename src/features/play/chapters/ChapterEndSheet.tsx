...
export function ChapterEndSheet({
  ...
  const displayTitle = (isEnglish && /[\u4e00-\u9fff]/.test(summary.title ?? "")) ? getChapterDisplayName(definition, chapterState, language) : (summary.title || getChapterDisplayName(definition, chapterState, language));
  return (
    <div className="absolute inset-x-3 bottom-[calc(7.8rem+env(safe-area-inset-bottom))] z-50">
      <section
        ...
        aria-label={`${formatChapterTitle(definition, chapterState, language)} ${isEnglish ? "chapter pause" : "章节留页"}`}
      >
        <div className="mb-4 flex items-start justify-between gap-3 border-b border-[#ded8ce] pb-3">
          <div className="min-w-0">
            <p className="vc-reading-serif text-[18px] leading-none text-[#4f706a]">
              {isEnglish ? "Chapter pause" : "本章留页"}
            </p>
            <h2 className="mt-2 vc-reading-serif text-[26px] font-semibold leading-tight text-[#174d46]">
              {displayTitle || formatChapterTitle(definition, chapterState, language)}
            </h2>
          </div>
          <button
            type="button"
            onClick={onDismiss}
            aria-label={isEnglish ? "Pause here" : "暂时停在这里"}
        ...
        <div className="mt-5 grid gap-2">
          ...
          {hasPreviousChapter ? (
            ...
            >
              {isEnglish ? "Review previous" : "回看上一章"}
            </button>
          ) : null}
        </div>
      </section>
