// src/lib/ai/tools/mechanicsIntentClassifier/index.ts
//
// per-world Mechanics Workflow intent 分类器入口。组合三层：
//
...
//
//   2. 纯关键词分类器（fallback），搬自 mechanicsIntentRouter.ts 的现有语义——
//      当 embedding 不可用（not_configured / timeout / 网络错误）时降级到这里，
//      保证暗月+星逆 Mechanics Workflow 路径在任意环境下都不退化到全 envelope。
//
...
import { bestCosineSimilarity } from "./cosine";
import { embedTextWithTimeout } from "./runtime";
...
/** 余弦相似度阈值：超过此值认为 mechanics 意图。调高 = 更严格（少进 Mechanics Workflow）。 */
export const DM_INTENT_EMBEDDING_THRESHOLD = 0.6;

/** 余弦相似度"模糊带"上限：超过此值即使未到阈值也返回 mechanics（rare 词）。 */
/** 在线 embedding 超时（毫秒）。 */
export const DM_INTENT_EMBEDDING_TIMEOUT_MS = 300;
...
export async function classifyIntent(
  ...
  // 2. keyword fallback
  const kw = classifyMechanicsIntentForWorld(userInput, worldId);
  return {
 ...
 * 用于：测试 / 极快路径（毫秒级 budget 不允许 500ms IO）/ 已经降级确定不会用 Mechanics Workflow 的场景。
 */
export function classifyIntentSync(userInput: string, worldId?: WorldId | string): IntentClassifierResult {
  const kw = classifyMechanicsIntentForWorld(userInput, worldId);
  return {
    classification: kw.classification,
    path: kw.classification === "mechanics" ? "keywords" : "ambiguous",
