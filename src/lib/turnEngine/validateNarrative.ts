...
const INVENTORY_ACQUISITION_PATTERN =
  /(捡起|拾起|收进口袋|放进口袋|装进背包|放入背包|收下了|得到了|获得了|塞进(?:了)?(?:口袋|兜里|裤兜|包里|背包)|揣进(?:了)?(?:口袋|兜里|怀里)|握紧(?:了)?(?:钥匙|纸条|照片|徽章|卡片|信件|信)|抽出(?:了)?(?:信封|纸条|信件|钥匙|照片|文件|笔记本)|拿起(?:了)?(?:钥匙|纸条|照片|信封|信件|徽章|卡片)|取下(?:了)?(?:钥匙|纸条|照片|徽章|卡片|信件|笔记本)|翻出(?:了)?(?:钥匙|纸条|照片|信封|徽章|卡片)|摸出(?:了)?(?:钥匙|纸条|照片|徽章|卡片)|把(?:这|那)?(?:把|柄|件)?(?:剑|刀|枪|武器|法器|它)(?:插|别|挂|系|佩)(?:在|到|上)(?:腰|腰间|腰侧|背后|身上)|(?:将|把)(?:它|这(?:把|件)?(?:剑|刀|枪|武器|法器)?)(?:加入|放入|装入|收进)(?:了)?(?:背包|行囊|物品栏)|(?:装备|佩戴)(?:上|了)?(?:它|这(?:把|件)?(?:剑|刀|枪|武器|法器)))/;
