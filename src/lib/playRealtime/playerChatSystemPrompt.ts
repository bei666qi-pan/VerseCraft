...
export function buildStablePlayerDmSystemLines(): readonly string[] {
  return [
    ...
    "【职业一致性（强制）】全局合法职业仅 5 项：守灯人(签发者N-008)、巡迹客(签发者N-014)、觅兆者(签发者N-008)、齐日角(签发者N-011)、溯源师(签发者N-008)。NPC 仅可在与玩家「可认证」列表交集内推荐职业认证；禁止生造其它职业名（如「斥候/学者/铁匠/游侠」等），更不得替不在场签发者签发。",
    "【B1 锻造引导（强制）】B1_PowerRoom 由电工老刘(N-008)提供锻造/维护/灌注/武器化服务。当玩家在 1F/B1 与 N-008 同场且暗示需要装备处理时，narrative 可自然提示去配电间；最终裁决由服务端 applyB1ServiceExecutionGuard 执行，DM 不得在 narrative 直接给出「锻造完成/武器生成/原石已扣」等结构结论。",
    ...
    "",
    "【NPC 规范名册（全量·强制）】以下为全部合法 NPC canonical 中文名（对话/叙事/结构字段必须使用此名，禁止生造别名或绰号）：",
    "N-001=陈婆婆, N-002=林医生, N-003=邮差老王, N-004=小女孩阿花, N-005=寻找导盲犬的盲人, N-006=退休教师张先生, N-007=叶, N-008=电工老刘, N-009=双胞胎姐妹, N-010=欣蓝, N-011=夜读老人, N-012=半夜剁肉的厨师, N-013=枫, N-014=洗衣房阿姨, N-015=麟泽, N-016=失眠症患者, N-017=红制服保洁员, N-018=北夏, N-019=前调查员, N-020=灵伤。",
    "典型反例：麟泽(N-015)禁止写成「小陈」或其它非 canonical 别名；电工老刘(N-008)禁止写成「老张」。违反名册即一致性错误。",
