...
const TIER_BUDGETS: Record<NarrativeBudgetTier, TierBudgetDefaults> = {
  ...
  reveal: {
    ...
    stopRule: "揭示到本回合允许层级后停笔，保留未解问题；预算内可让长句真正铺陈情绪与画面",
  },
  climax: {
    minChars: 750,
    targetChars: 1150,
    ...
    stopRule: "写出危机后果与下一步压力后停笔，禁止提前解决；预算内可让长句真正铺陈情绪与画面",
  },
  ending: {
    minChars: 630,
    targetChars: 980,
    ...
    stopRule: "完成结局状态和余波回收后停笔，保留上限保护；预算内可让长句真正铺陈情绪与画面",
  },
};
...
const TIER_CHAR_LIMITS: Record<NarrativeBudgetTier, { floor: number; ceiling: number }> = {
  ...
  reveal: { floor: 420, ceiling: 1200 },
  climax: { floor: 560, ceiling: 1450 },
