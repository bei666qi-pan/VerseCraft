...
import { advanceChapterPacing } from "./chapterPacingController";
import { createInitialChapterPacingState } from "./types";

function mkSignals(overrides: Partial<ChapterPacingSignals> = {}): ChapterPacingSignals {
  return {
    nowTurn: 0,
...
test("chapter reasoner pushes a meaningful choice when structured progress stalls", () => {
  const pacing = createInitialChapterPacingState(0);
  pacing.stallCount = 2;
  pacing.chapter = { ...pacing.chapter, startedTurn: 0, chapterPhase: "rising" };
  const out = planChapterStep({
    pacing,
...
test("chapter reasoner leaves aftershock after a reveal under pressure", () => {
  const pacing = createInitialChapterPacingState(0);
  pacing.chapter = { ...pacing.chapter, startedTurn: 0, chapterPhase: "reveal" };
  const out = planChapterStep({
    pacing,
    signals: mkSignals({
      nowTurn: 6,
...
test("chapter close decision requires player choice echo, except first chapter with explicit intervention", () => {
  ...
  const secondChapter = createInitialChapterPacingState(0).chapter;
  ...
  const firstWithoutIntervention = evaluateChapterCloseDecision({
    chapter: { ...createInitialChapterPacingState(0).chapter, startedTurn: 0, minTurns: 2 },
...
test("chapter reasoner update generates a next chapter seed after a valid close decision", () => {
  const pacing = createInitialChapterPacingState(0);
  pacing.openHookCodes = ["next_hook"];
  ...
  assert.ok(out.state.chapter.nextChapterSeed);
  assert.equal(out.state.chapter.nextChapterSeed?.title, "潮湿门缝");
  assert.ok(out.state.chapter.nextChapterSeed?.inheritedThreadIds.includes("next_hook"));
});
