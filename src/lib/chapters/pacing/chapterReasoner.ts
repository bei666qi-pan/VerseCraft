...
function matchesChapter(entry: MemorySpineEntry, chapter: PacingChapterState): boolean {
...
function hasConflictSignal(signals: ChapterPacingSignals): boolean {
...
function nextChapterTitleCandidate(chapter: PacingChapterState, shouldClose: boolean): string | null {
...
export function evaluateChapterCloseDecision(args: {
  ...
  directorPlan: ChapterPacingPlan;
...
function selectChapterPhase(args: {
  ...
  const { pacing, chapter, signals } = args;
  ...
  const mustOfferChoice = signals.stalled || (pacing.stallCount ?? 0) >= 2;
...
export function planChapterStep(args: {
  ...
  const chapter = args.pacing.chapter;
  ...
  const provisionalChapter: PacingChapterState = {
    ...chapter,
    openThreadIds,
    resolvedThreadIds,
