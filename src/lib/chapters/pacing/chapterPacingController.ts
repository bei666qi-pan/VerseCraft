...
import { detectChapterPacingSignals } from "./signals";
...
import { planChapterPacing } from "./planner";
import {
  createInitialChapterPacingState,
  createInitialPacingChapterState,
  type ChapterCloseDecision,
  type PacingChapterState,
  type ChapterPhase,
  type ChapterPacingPlan,
  type NextChapterSeed,
  type ChapterPacingState,
} from "./types";
import { shouldChapterPacingBuildNextChapterSeed } from "./types";

export type { ChapterPacingPlan, ChapterPacingState } from "./types";
...
function applyChapterBridge(
  ...
  if (bridgeId && bridgeId !== chapter.currentChapterId) {
    return createInitialPacingChapterState(nowTurn, {
...
function normalizePacingChapterState(
  raw: unknown,
  nowTurn: number,
  bridge?: ChapterBridgeInput
): PacingChapterState {
  const base = createInitialPacingChapterState(nowTurn, bridge ?? {});
  const o = asRecord(raw);
  if (!o) return base;
  const normalized: PacingChapterState = {
    ...base,
    v: 1,
...
export function normalizeChapterPacingState(
  raw: unknown,
  nowTurn: number,
  bridge?: ChapterBridgeInput
): ChapterPacingState {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    const empty = createInitialChapterPacingState(nowTurn);
    return {
      ...empty,
      chapter: normalizePacingChapterState(undefined, nowTurn, bridge),
    };
  }
  const o = raw as Record<string, unknown>;
  const base = createInitialChapterPacingState(nowTurn);
  return {
    ...base,
    v: 1,
    ...
    recentProgressTurns: Array.isArray(o.recentProgressTurns) ? uniq(o.recentProgressTurns as string[], 8).map((x) => clampInt(Number(x), 0, 999999)) : [],
    recentPeakTurn: clampInt(o.recentPeakTurn, 0, 999999),
    openHookCodes: Array.isArray(o.openHookCodes) ? uniq(o.openHookCodes as string[], 12) : [],
    falseCalmTurns: clampInt(o.falseCalmTurns, 0, 99),
    pressureBudget: clampInt(o.pressureBudget, 0, 100),
    escapePressureBand: o.escapePressureBand === "high" || o.escapePressureBand === "mid" ? o.escapePressureBand : "low",
    chapter: normalizePacingChapterState(o.chapter, nowTurn, bridge),
  };
}
...
function buildChapterCloseDecision(input: {
  chapter: PacingChapterState;
  signals: ReturnType<typeof detectChapterPacingSignals>;
  ...
  const chapter: PacingChapterState = {
    ...input.chapter,
...
function advancePacingChapterState(args: {
  ...
  const closeCandidate =
    baseCloseCandidate?.shouldClose && modelTitleCandidate
      ...
      : baseCloseCandidate;
  const triggerBuildSeed = shouldChapterPacingBuildNextChapterSeed({
    chapter: args.chapter,
    nowTurn: args.nowTurn,
  });
  ...
  const chapterWithCandidates: PacingChapterState = {
    ...args.chapter,
    openThreadIds,
    resolvedThreadIds,
    keyChoiceIds,
  ...
  return planChapterStep({
    pacing: { ...args.pacing, chapter: chapterWithCandidates },
    signals: args.signals,
    memoryEntries: args.postMemoryEntries,
    nowTurn: args.nowTurn,
  });
...
export function advanceChapterPacing(args: {
  stateRaw: unknown;
  nowTurn: number;
  ...
  const director0Raw = normalizeChapterPacingState(args.stateRaw, nowTurn);
  const director0: ChapterPacingState = {
    ...director0Raw,
    chapter: applyChapterBridge(director0Raw.chapter, args.chapter, nowTurn),
  };
  const signals = detectChapterPacingSignals({
    pacing: director0,
    nowTurn,
    pre: args.pre,
    post: args.post,
  ...
  const director1: ChapterPacingState = {
    ...director0,
    stallCount,
    tension,
    falseCalmTurns,
  ...
  const plan = planChapterPacing({ pacing: director1, signals });

  const chapterStep = advancePacingChapterState({
    pacing: director1,
  ...
  const director2: ChapterPacingState = {
    ...director1,
    chapter,
    recentPeakTurn: plan.beatMode === "peak"
      ? nowTurn
