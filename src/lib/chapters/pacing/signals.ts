...
import type { ChapterPacingState } from "./types";

export type ChapterPacingSignals = {
  nowTurn: number;
...
export function detectChapterPacingSignals(args: {
  ...
  const moved = Boolean(preLoc && postLoc && preLoc !== postLoc);
  ...
  const openHooks = uniq(args.pacing.openHookCodes ?? [], 12);
  ...
  const falseCalmRisk = !highPressure && hooksReady && (args.pacing.falseCalmTurns ?? 0) >= 2;
