...
describe("retrieveWorldKnowledge", () => {
  it("精确命中优先级：当前位置与近期实体会被提升", () => {
    ...
    expect(out[0]?.fact.identity.factKey).toBe("npc:N-003:chunk:0");
  });

  it("私有世界观只对 owner 生效：scope SQL 含 owner 约束", () => {
    ...
    expect(filter.sql).toContain("owner_user_id");
    expect(filter.params[0]).toBe("u_1");
  });

  it("session facts 仅对应会话可见：scope SQL 含 session key 前缀", () => {
    const req: RuntimeLoreRequest = {
    ...
    const filter = mergeScopeFilterSql(req);
    expect(filter.sql).toContain("retrieval_key LIKE");
    expect(String(filter.params[1]).startsWith("session:s_9:")).toBe(true);
  });

  it("热点地点检索结果稳定：同输入 fingerprint 一致", () => {
    const req: RuntimeLoreRequest = {
    ...
    expect(p1.entitiesFingerprint).toBe(p2.entitiesFingerprint);
  });
});
