...
import { getDmToolDefinitions, getReadonlyDmToolDefinitions, getWriteDmToolDefinitions, DM_TOOL_REGISTRY } from "./dmToolHandlers";
import assert from "node:assert/strict";

// Import schemas
import { DM_TOOL_SCHEMAS, ALL_DM_TOOL_NAMES, READONLY_DM_TOOL_NAMES, WRITE_DM_TOOL_NAMES } from "./dmToolSchemas";
...
describe("DM Agent Tool Schemas", () => {
  it("所有工具都有有效的 Schema", () => {
    for (const name of ALL_DM_TOOL_NAMES) {
      const schema = DM_TOOL_SCHEMAS[name];
  ...
  it("只读工具不改变状态", () => {
    for (const name of READONLY_DM_TOOL_NAMES) {
      const schema = DM_TOOL_SCHEMAS[name];
  ...
  it("写工具会改变状态", () => {
    for (const name of WRITE_DM_TOOL_NAMES) {
      const schema = DM_TOOL_SCHEMAS[name];
...
describe("Agent Configuration", () => {
  ...
  it("总预算不超过合理范围", () => {
    assert.ok(DM_AGENT_DEFAULTS.TOTAL_BUDGET_MS >= 10_000);
...
describe("Tool Access Control", () => {
  ...
  it("所有工具定义都有非空 description", () => {
    // getDmToolDefinitions imported at top of file
    const allDefs = getDmToolDefinitions();
...
describe("Tool Timeout Protection", () => {
  it("工具超时常量在合理范围内", () => {
    // 验证默认超时配置
    assert.strictEqual(DM_AGENT_DEFAULTS.PER_TOOL_TIMEOUT_MS, 3000);
    assert.strictEqual(DM_AGENT_DEFAULTS.TOTAL_BUDGET_MS, 30000);
  });

  it("每个工具注册都有超时设置", () => {
    // DM_TOOL_REGISTRY imported at top of file
    for (const [name, reg] of Object.entries(DM_TOOL_REGISTRY)) {
  ...
  it("单工具超时不应超过总预算", () => {
    ...
    for (const [name, reg] of Object.entries(DM_TOOL_REGISTRY)) {
      assert.ok(
        reg.meta.timeoutMs <= DM_AGENT_DEFAULTS.TOTAL_BUDGET_MS,
        `Tool ${name} timeoutMs (${reg.meta.timeoutMs}) exceeds total budget (${DM_AGENT_DEFAULTS.TOTAL_BUDGET_MS})`
      );
    }
  });
