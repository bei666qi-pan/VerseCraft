// src/lib/ai/tools/mechanicsToolHandlers.ts
/**
 * Mechanics Workflow 工具处理器
 *
...
} from "./mechanicsServerStateAdapter";
...
/** 从当前游戏状态构建处理器上下文 */
function getServerState(ctx: MechanicsContext) {
...
async function handleConsumeMaterials(
  ...
  const result: MechanicsToolResult = {
    ok: true,
...
function buildToolDefinition(schemaName: string): ToolDefinition {
  const schema = MECHANICS_TOOL_SCHEMAS[schemaName as keyof typeof MECHANICS_TOOL_SCHEMAS];
  return {
    type: "function",
    function: {
...
/** 获取所有工具定义（用于发送给模型） */
export function getMechanicsToolDefinitions(): ToolDefinition[] {
  return Object.values(MECHANICS_TOOL_REGISTRY).map((reg) => reg.definition);
}

/** 获取只读工具定义 */
export function getReadonlyMechanicsToolDefinitions(): ToolDefinition[] {
  ...
  return Object.entries(MECHANICS_TOOL_REGISTRY)
    .filter(([name]) => readonlyNames.has(name))
    .map(([, reg]) => reg.definition);
}

/** 获取状态变更工具定义 */
export function getWriteMechanicsToolDefinitions(): ToolDefinition[] {
