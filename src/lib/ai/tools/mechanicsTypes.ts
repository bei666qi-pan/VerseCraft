// src/lib/ai/tools/mechanicsTypes.ts
/**
 * Mechanics Workflow 类型定义
 *
 * 核心概念：
 * - Mechanics Workflow 是一个受限的工具调用 Agent，不是自由循环的自主 Agent
...
/** 工具操作分类 */
export type MechanicsToolCategory =
  | "read_player_state"
  | "read_world_state"
  | "quest_operation"
  | "forge_operation"
...
/** 工具读写性质 */
export type MechanicsToolAccess = "read" | "write";
...
/** 工具执行成功结果 */
export interface MechanicsToolSuccess<T = unknown> {
  ok: true;
  data: T;
  /** 供叙事层使用的结构化摘要 */
...
/** 工具执行失败结果 */
export interface MechanicsToolFailure {
  ok: false;
  error: string;
  /** 失败原因码 */
...
export type MechanicsToolResult<T = unknown> = MechanicsToolSuccess<T> | MechanicsToolFailure;

/** 工具错误码 */
export type MechanicsToolErrorCode =
  ...
  | "world_mismatch"           // 工具作用域与当前世界不一致
  | "timeout"                  // 操作超时
  | "internal_error";          // 内部错误

// ============================================================
// Mechanics Workflow Tool Metadata
// ============================================================

export interface MechanicsToolMeta {
  name: string;
  description: string;
  category: MechanicsToolCategory;
  access: MechanicsToolAccess;
  /** 是否只读 */
  readonly: boolean;
  /** 是否改变游戏状态 */
...
// ============================================================
// Mechanics Workflow Execution Context
// ============================================================
...
export interface MechanicsContext {
  ...
  /** 当前世界 ID */
  worldId: string;
  /** Workflow-owned execution limits. */
  limits: MechanicsExecutionLimits;
  /** Abort signal */
  signal?: AbortSignal;
  /** 服务端游戏状态（用于工具处理器读取真实数据） */
  serverGameState?: import("./mechanicsServerStateAdapter").ServerGameState;
}

export interface MechanicsExecutionLimits {
...
// ============================================================
// Mechanics Workflow Turn Result
// ============================================================
...
export interface MechanicsTurnResult {
  ...
  narrative: string;
  /** 工具调用追踪 */
  toolTrace: MechanicsToolCallTrace[];
  /** 状态变更摘要 */
  stateDelta: MechanicsStateDelta;
  /** 是否调用了工具 */
  toolsUsed: boolean;
  /** 总耗时（ms） */
...
export interface MechanicsToolCallTrace {
  toolName: string;
  ok: boolean;
  latencyMs: number;
  error?: string;
...
export interface MechanicsStateDelta {
  questsIssued: number;
...
export interface MechanicsToolRegistration {
  meta: MechanicsToolMeta;
  definition: ToolDefinition;
  handler: (args: Record<string, unknown>, ctx: MechanicsContext) => Promise<MechanicsToolResult>;
}

// ============================================================
// Mechanics Workflow Configuration
// ============================================================

export const MECHANICS_DEFAULTS = {
  MAX_TOOL_ROUNDS: 2,
  MAX_TOOL_ROUNDS_HARD_CAP: 2,
...
// ============================================================
// Mechanics Workflow System Prompt Fragments
// ============================================================

export const MECHANICS_TOOL_INSTRUCTIONS = [
  "【Mechanics Workflow 工具使用规则（强制）】",
  "你负责基于注册规则处理需要状态读写的玩家行动。你必须遵守以下规则：",
  "",
