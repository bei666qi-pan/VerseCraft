...
/**
 * Mechanics Workflow 工具 JSON Schema 服务端校验测试
 *
...
import { MECHANICS_TOOL_SCHEMAS, ALL_MECHANICS_TOOL_NAMES } from "./mechanicsToolSchemas";

describe("Schema Validation — All Tools", () => {
  for (const toolName of ALL_MECHANICS_TOOL_NAMES) {
    it(`${toolName}: schema 存在且有效`, () => {
      const schema = MECHANICS_TOOL_SCHEMAS[toolName];
...
describe("Schema Validation — Unknown Tool Rejection", () => {
  it("未知工具名不在注册表中", () => {
    ...
    for (const name of unknownNames) {
      assert.ok(!(name in MECHANICS_TOOL_SCHEMAS), `Unknown tool '${name}' must not be in registry`);
    }
  });
...
describe("Schema Validation — Type Enforcement", () => {
  ...
  it("update_quest_progress new_status 有合法枚举值", () => {
    const schema = MECHANICS_TOOL_SCHEMAS.update_quest_progress;
    const props = (schema.parameters as any).properties as Record<string, any>;
...
describe("Schema Validation — Max Tool Result Size", () => {
  it("工具结果大小限制防止信息泄露", () => {
    ...
    // MECHANICS_DEFAULTS imported at top of file
    assert.ok(MECHANICS_DEFAULTS.MAX_TOOL_RESULT_CHARS > 0);
    assert.ok(MECHANICS_DEFAULTS.MAX_TOOL_RESULT_CHARS <= 10_000);
  });
});
