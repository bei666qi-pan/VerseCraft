// src/lib/ai/tools/dmToolCache.ts
/**
 * DM Agent 只读工具结果缓存
 * 
...
class ToolResultCache {
  ...
  get(key: string): DmToolResult | null {
  ...
  set(key: string, result: DmToolResult): void {
    // LRU 淘汰
 ...
 * 尝试从缓存获取工具结果
 */
export function getCachedToolResult(toolName: string, args: Record<string, unknown>): DmToolResult | null {
 ...
 * 将工具结果写入缓存
 */
export function setCachedToolResult(toolName: string, args: Record<string, unknown>, result: DmToolResult): void {
  // 只缓存成功结果
  if (!result.ok) return;
