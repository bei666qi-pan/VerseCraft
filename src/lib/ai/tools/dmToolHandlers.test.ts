...
import { DM_TOOL_REGISTRY, getDmToolDefinitions, getReadonlyDmToolDefinitions } from "./dmToolHandlers";
import type { DmAgentContext, DmToolFailure } from "./dmAgentTypes";
import { DM_TOOL_SCHEMAS } from "./dmToolSchemas";

// ── Shared test context ──

function makeCtx(overrides: Partial<DmAgentContext> = {}): DmAgentContext {
  return {
...
test("DM_TOOL_REGISTRY includes lookup_location and check_npc_stock", () => {
  assert.ok("lookup_location" in DM_TOOL_REGISTRY, "lookup_location missing from registry");
  assert.ok("check_npc_stock" in DM_TOOL_REGISTRY, "check_npc_stock missing from registry");
});

test("DM_TOOL_REGISTRY count matches DM_TOOL_SCHEMAS", () => {
  const registryKeys = Object.keys(DM_TOOL_REGISTRY).sort();
  const schemaKeys = Object.keys(DM_TOOL_SCHEMAS).sort();
  assert.deepStrictEqual(registryKeys, schemaKeys);
});

test("getDmToolDefinitions includes lookup_location and check_npc_stock", () => {
  const defs = getDmToolDefinitions();
  const names = defs.map((d) => d.function.name);
  assert.ok(names.includes("lookup_location"));
  assert.ok(names.includes("check_npc_stock"));
});

test("getReadonlyDmToolDefinitions includes lookup_location and check_npc_stock", () => {
  const defs = getReadonlyDmToolDefinitions();
  const names = defs.map((d) => d.function.name);
  assert.ok(names.includes("lookup_location"));
  assert.ok(names.includes("check_npc_stock"));
});
...
test("lookup_location with exact room node match", async () => {
  const handler = DM_TOOL_REGISTRY.lookup_location.handler;
...
test("lookup_location with non-existent location returns error", async () => {
  ...
  assert.ok(!result.ok);
  const fail = result as DmToolFailure;
...
test("check_npc_stock with valid npc_id returns npc info", async () => {
  const handler = DM_TOOL_REGISTRY.check_npc_stock.handler;
...
test("lookup_location tool definition has correct schema", () => {
  const reg = DM_TOOL_REGISTRY.lookup_location;
...
test("check_npc_stock tool definition has correct schema", () => {
  const reg = DM_TOOL_REGISTRY.check_npc_stock;
