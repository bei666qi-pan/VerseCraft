...
import {
  DM_TOOL_SCHEMAS,
  ALL_DM_TOOL_NAMES,
  READONLY_DM_TOOL_NAMES,
  WRITE_DM_TOOL_NAMES,
} from "./dmToolSchemas";

// ── Schema registry completeness ──

test("DM_TOOL_SCHEMAS contains all expected tools", () => {
  const names = Object.keys(DM_TOOL_SCHEMAS);
...
test("ALL_DM_TOOL_NAMES matches DM_TOOL_SCHEMAS keys", () => {
  const schemaKeys = Object.keys(DM_TOOL_SCHEMAS).sort();
  const allNames = [...ALL_DM_TOOL_NAMES].sort();
  assert.deepStrictEqual(allNames, schemaKeys);
});

test("READONLY_DM_TOOL_NAMES contains only read tools", () => {
...
test("WRITE_DM_TOOL_NAMES contains only write tools", () => {
  const expectedWrite = [
  ...
  assert.deepStrictEqual([...WRITE_DM_TOOL_NAMES].sort(), [...expectedWrite].sort());
});

test("readonly + write = all tool names", () => {
  const combined = new Set([...READONLY_DM_TOOL_NAMES, ...WRITE_DM_TOOL_NAMES]);
  assert.equal(combined.size, ALL_DM_TOOL_NAMES.length);
...
test("readonly and write sets are disjoint", () => {
  const readonlySet = new Set(READONLY_DM_TOOL_NAMES);
...
test("every tool schema has meta with name and description", () => {
  for (const [key, schema] of Object.entries(DM_TOOL_SCHEMAS)) {
...
test("grant_item schema has expected params", () => {
  const schema = DM_TOOL_SCHEMAS["grant_item"];
  const props = schema.parameters.properties;
  assert.ok(props);
  assert.ok("idempotency_key" in props);
