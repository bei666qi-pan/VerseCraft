...
test("高魅力：职能面+异常伏笔成立；通用 lore 不得明牌耶里校籍", () => {
  for (const id of MAJOR_NPC_IDS) {
    const p = CORE_NPC_PROFILES_V2.find((x) => x.id === id);
    assert.ok(p, id);
    const ss = p!.interaction.surfaceSecrets.join("；");
    assert.ok(ss.includes("公寓职能面") && ss.includes("异常感"), `${id} profile surfaceSecrets`);
    assert.ok(!ss.includes("耶里"), `${id} surfaceSecrets 不得含耶里`);
    const row = NPCS.find((n) => n.id === id);
    assert.ok(row, id);
    if (row!.lore.includes("公寓职能面")) {
      assert.ok(row!.lore.includes("异常感"), `${id} NPCS.lore 含异常伏笔`);
      assert.ok(!row!.lore.includes("耶里"), `${id} NPCS.lore 不得含耶里`);
    }
  }
});
