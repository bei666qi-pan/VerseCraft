...
export const FLOORS: readonly { id: FloorId; label: string; description: string }[] = [
  ...
  { id: "4", label: "4 楼", description: "401 室（张先生）、402 室、走廊尽头。周伯徘徊此处呼唤大黄。" },
  { id: "5", label: "5 楼", description: "501 室、502 室、503 画室。住户口耳相传那里像能躲一躲的地方，墙上未完成的自画像特别多，画中人的轮廓总像比你多转了一点角度。" },
  { id: "6", label: "6 楼", description: "601 室、602 室（阿织/阿绣）、楼梯间。章嫂整夜喃喃低语。" },
  { id: "7", label: "7 楼", description: "701 室、走廊长椅（夜读老人）、厨房（陶师傅）、紧闭门扉区。公寓最深层的秘密汇聚于此。" },
];
...
export const NPC_SOCIAL_GRAPH: Record<string, NpcSocialProfile> = {
  ...
  "N-002": {
    ...
    scheduleBehavior: "全天驻守诊室，偶尔去 4 楼探视周伯",
    relationships: {
      "N-005": "定期为其检查（实为采集数据）",
      "N-012": "互不干涉的冷淡邻居",
    ...
    core_desires: "用病历证明自己仍是「医生」；借周伯的检查数据寻找逆转消化的可能。",
    immutable_relationships: [
      "对周伯（N-005）定期采集数据，隐瞒真相",
      "与厨师（N-012）互不干涉，心照不宣",
      "对认知腐蚀者（A-003）有刻骨恐惧",
    ],
  ...
  "N-004": {
    ...
    core_desires: "永远不被陈婆婆遗忘；永远不要玩阿织的捉迷藏。",
    immutable_relationships: [
  ...
  "N-009": {
    ...
    weakness: "在两人间做选择会导致暴走；共鸣水晶碎裂则阿织/阿绣之一消亡",
    ...
    fixed_lore:
      "住户登记上只有一名儿童。姐姐阿织说妹妹阿绣是「后来出现的」，妹妹阿绣说姐姐阿织是「镜子里走出来的」。她们共用一颗心脏——真的。她们来自与倒行者相同的镜像维度，是公寓消化过程中的「残留」。北夏（N-018）与她们无旧契；镜像分辨更多依赖楼内其它反光与诡异规则。",
  ...
  "N-010": {
    ...
    weakness: "（静态占位）patch 覆盖为欣蓝双层档案；勿在此写「科层物业经理」单一壳。",
    scheduleBehavior: "（静态占位）patch 覆盖",
    relationships: {},
    fixed_lore: "（静态占位）patch 覆盖为欣蓝双层档案；勿在此写「物业经理」旧设定。",
    core_desires: "（静态占位）patch 覆盖",
  ...
  "N-014": {
    ...
    relationships: {
      "N-008": "地下一层互助同盟，彼此信任",
      "N-017": "恐惧红姨（她知道茶壶里是什么）",
      "N-020": "灵伤来取洗净制服时像广播试麦，她下意识把音量拧小",
      "N-015": "麟泽雨痕外套滴在洗衣房门口，她递干布从不问对方住哪一层",
    },
    fixed_lore:
      "防污染应急协议时期的后勤人员。她知道红色自来水的真相，也知道「静置 12 小时」是为了让胃酸稀释。她选择留下因为「总得有人洗那些洗不干净的东西」。沾血衣物会触发她的旧习与创伤。她知道红姨茶壶里是什么——浓缩消化液。",
    core_desires: "守护地下一层安全区；与老刘一起维持最后的「正常」；永远不要再碰红姨的茶壶。",
    immutable_relationships: [
      "与电工老刘（N-008）是地下一层同盟，彼此信任",
      "恐惧红姨（N-017），知道茶壶内是浓缩消化液",
  ...
  "N-016": {
    ...
    relationships: {
      "N-009": "阿织/阿绣的存在让他更加不安",
      "A-006": "倒行者与错层门牌让他分不清上下",
      "N-002": "拒绝林医生的安眠药处方",
      "N-013": "夜半听见七层有人笑得太像广播剧，他更不敢睡",
    },
    fixed_lore:
      "他的失眠是 6F 镜像与错层污染的早期症状；「10 楼走廊」只是 6F 楼梯间投影，不是真实楼层。他听见的吞咽声来自楼体消化。保持清醒只能延迟同化。安眠药会让他在梦中被错层吞没。他拒绝林医生的处方。阿织/阿绣的存在加剧他的不安，倒行者让他分不清上下。",
    core_desires: "永远不要入睡；找到能陪他保持清醒的人；在被同化前逃出公寓。",
    immutable_relationships: [
      "阿织/阿绣（N-009/N-021）让他更加不安",
