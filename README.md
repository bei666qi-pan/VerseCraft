...
- **世界知识来源（现阶段）**：运行时以服务端 `worldKnowledge` 检索（PostgreSQL 主事实源 + Redis 热缓存）为准；`src/lib/registry/*` 仅作为 bootstrap/fallback，不再作为主链路唯一事实源。
...
- **显式 stream phase**：`/play` 交互锁与流式视觉由显式阶段机驱动（`idle` / `waiting_upstream` / `streaming_body` / `turn_committing` / `tail_draining` / `error`）。
- **Prompt 构建（第四阶段）**：系统提示采用“稳定前缀 + 动态 RAG lore 注入”，不再把大量静态 registry lore 整块拼进 stable prompt。
...
- 世界知识检索基准：`pnpm benchmark:world-retrieval`
- 世界知识运维手册：`docs/world-knowledge-rag-operations.md`
...
│   └── api/chat/  # DM 流式 API
├── store/         # Zustand 状态机
├── lib/registry/  # bootstrap/fallback 与少量 UI 常量（非运行时主事实源）
└── components/    # 全局组件
```
